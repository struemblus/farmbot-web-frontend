...
export function settingToggle(name, bot) {
  return function(dispatch) {
    ...
    let promise = bot.updateCalibration(packet)
...
export function commitSettingsChanges() {
  ...
  let packet = _({})
                 ...
                 .value()
  let promise = bot.current.updateCalibration(packet);
  return function(dispatch) {
    return promise.then(
...
export function commitAxisChanges() {
  ...
  let packet = _({})
                 ...
                 .value()
  let promise = bot.current.moveAbsolute(packet);
  return function(dispatch) {
    return promise.then(
...
export function readStatus() {
  let promise = bot.current.readStatus();
  return function(dispatch) {
...
export function sendCommand(payload) {
  if (!bot.current.offline()) {
    let method = bot.current[payload.name];
    let result = method.call(bot.current, payload);
    return (dispatch) => {
      return result.then(
